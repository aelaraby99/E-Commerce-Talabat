...
namespace Talabat.APIs.Controllers
{
    ...
    public class ProductsController : BaseApiController
    {
        ...
        public async Task<ActionResult<IReadOnlyList<Pagination<ProductToReturnDto>>>> GetProducts([FromQuery] ProductSpecParams productSpec)
        {
            ...
            var mappedData = mapper.Map<IReadOnlyList<Product>, IReadOnlyList<ProductToReturnDto>>(Products);
            var CountSpec = new ProductWithFilterationForCountSpecification(productSpec);
            var Count = await _unitOfWork.Repository<Product>().GetCountWithSpecAsync(CountSpec);
            return Ok(new Pagination<ProductToReturnDto>(productSpec.PagIndex, productSpec.PageSize, Count, mappedData));
        }
        ...
        [HttpGet("types")] //api/products/categories
